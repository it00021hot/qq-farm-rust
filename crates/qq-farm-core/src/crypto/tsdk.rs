...
fn extract_exports(instance: &Instance, store: &mut Store<HostState>) -> Result<Exports> {
    ...
    let mut get_required = |name: &str| -> Result<Func> {
        instance
            .get_func(&mut *store, name)
            .ok_or_else(|| Error::crypto(format!("missing required export: {name}")))
    };

    Ok(Exports {
        memory,
        alloc: get_required("A")?,
        free: get_required("B")?,
        x: get_required("x")?,
        g: get_required("G")?,
        encrypt: get_required("ba")?,
        decrypt: get_required("ca")?,
        decrypt_strings: instance
