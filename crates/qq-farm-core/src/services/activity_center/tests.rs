...
fn activity_error_display() {
    let e = ActivityError { code: ActivityErrorCode::ShopUnavailable, message: "test".to_string() };
...
fn item_dto_from_core_item() {
    let i = CoreItem { id: 1002, count: 50, ..Default::default() };
    let dto = item_dto(&i);
...
fn pass_dto_basic() {
    ...
    assert_eq!(dto.nodes[0].rewards[0].image, "/game-config/seed_images_named/seed_images/1.webp");
    let v = serde_json::to_value(&dto).unwrap();
    assert!(v.get("nodes").and_then(|n| n.as_array()).is_some());
    assert_eq!(
        v["nodes"][0]["rewards"][0]["image"],
        "/game-config/seed_images_named/seed_images/1.webp"
    );
}
...
fn activity_error_into_core_error() {
    let e = ActivityError { code: ActivityErrorCode::ShopUnavailable, message: "x".to_string() };
...
fn qingmei_already_claimed_message_detects_code_and_text() {
    assert!(is_qingmei_already_claimed_message("gateway error: x.y code=1034014 already"));
    assert!(is_qingmei_already_claimed_message("今日青梅种子已经领取，无需重复领取"));
...
fn encode_query_activity_request() {
    let req = QueryActivityRequest { activity_id: 100, operate_type: QUERY_SHOP_OPERATE_TYPE };
