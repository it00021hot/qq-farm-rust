...
use crate::constants::{
    ACTIVITY_SERVICE, EXCHANGE_SHOP_OPERATE_TYPE, QUERY_SHOP_OPERATE_TYPE, SHOP_ACTIVITY_TYPE,
};
use crate::error::Result;
use crate::proto::generated::gamepb::activitypb::{ActivityOperateReply, ExchangeShopRequest};
use crate::proto::generated::gamepb::seasonpb::GetSeasonInfoReply;
use crate::services::warehouse::WarehouseService;

use super::dto::{
    extract_goods, find_season_activity, item_from_id, normalize_shop_from_reply, positive_decimal,
    read_bag_balances,
};
...
impl ActivityCenterService {
    ...
    pub(crate) async fn shop_from_season_reply(
        ...
        let shop_activity =
            find_season_activity(season_reply, SHOP_ACTIVITY_TYPE).ok_or_else(|| {
                ActivityError {
                    code: ActivityErrorCode::ShopUnavailable,
                    message: "当前赛季未发现活动商店".to_string(),
                }
            })?;
        let reply = self.query_shop_catalog(shop_activity.activity_id).await?;
        ...
        Ok(normalize_shop_from_reply(season_reply, shop_activity, &reply, balances.as_ref()))
    }
    ...
    async fn query_shop_catalog(&self, activity_id: i64) -> Result<ActivityOperateReply> {
        ...
        if reply.operate_type != QUERY_SHOP_OPERATE_TYPE {
            return Err(ActivityError {
                code: ActivityErrorCode::ShopResponseInvalid,
                message: format!("活动商店查询返回了未知操作类型: {}", reply.operate_type),
            }
            .into());
        }
    ...
    pub async fn exchange_star_sand_goods(
        ...
        let goods_id =
            positive_decimal(goods_id_input, ActivityErrorCode::InvalidShopGoodsId, "goodsId")?;
        let count =
            positive_decimal(count_input, ActivityErrorCode::InvalidExchangeCount, "count")?;
        if count <= 0 {
            return Err(ActivityError {
        ...
        let catalog_reply = self.query_shop_catalog(shop_activity.activity_id).await?;
        ...
        let balances =
            read_bag_balances(warehouse, &[currency_id]).await.ok_or_else(|| ActivityError {
        ...
        let normalized_goods =
            shop_before.goods.iter().find(|g| g.id == goods_id).ok_or_else(|| ActivityError {
                code: ActivityErrorCode::ShopGoodsNotFound,
                message: "活动商店中未找到指定商品".to_string(),
            })?;
        ...
        if reply.operate_type != EXCHANGE_SHOP_OPERATE_TYPE {
            return Err(ActivityError {
                code: ActivityErrorCode::ShopResponseInvalid,
                message: format!("活动商店兑换返回了未知操作类型: {}", reply.operate_type),
            }
            .into());
        }
        if reply.data.as_ref().and_then(|d| d.catalog.as_ref()).is_none() {
            return Err(ActivityError {
                code: ActivityErrorCode::ShopResponseInvalid,
        ...
        let total_item_count = if unit_item_count > 0 { unit_item_count * count } else { 0 };
