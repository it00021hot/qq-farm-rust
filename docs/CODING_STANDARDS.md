...
| `qq-farm-desktop` | Tauri IPC → 调 **同一套** app；默认内嵌 RuntimeEngine | 把 Tauri 泄漏进 app；经 localhost HTTP |

**硬性规则：** 只维护桌面版（`qq-farm-server` 已删除）；`qq-farm-app` **禁止**依赖 `tauri`。

## 常量

- 游戏协议相关（RPC 服务名、活动/道具 ID、操作码、TTL）放 `qq_farm_core::constants`。
- 进程/部署配置放各入口的 `*Config`，不写死在 handler。
