...
const fields = computed(() => props.columns.filter((column): column is Field =>
  'key' in column && !('children' in column) && column.key !== 'index'
));
const primary = computed(() => fields.value.filter(column => props.primaryKeys.includes(String(column.key))));
const secondary = computed(() => fields.value.filter(column => !props.primaryKeys.includes(String(column.key)) && column.key !== 'operate'));
const actions = computed(() => fields.value.find(column => column.key === 'operate'));
const allChecked = computed(() => props.data.length > 0 && props.data.every(row => checked.value.includes(props.rowKey(row))));
const partiallyChecked = computed(() => !allChecked.value && props.data.some(row => checked.value.includes(props.rowKey(row))));
...
function selectPage(value: boolean) {
  ...
  checked.value = value ? [...new Set([...checked.value, ...pageKeys])] : checked.value.filter(key => !pageKeys.includes(key));
}
...
<template>
  <NSpin :show="loading">
    <div class="mobile-record-list flex flex-col gap-12px">
      <NCheckbox v-if="data.length" :checked="allChecked" :indeterminate="partiallyChecked" @update:checked="selectPage">
      ...
      <article v-for="(row, index) in data" :key="rowKey(row)" class="record-card">
        <div class="flex items-start gap-12px">
          <NCheckbox :checked="checked.includes(rowKey(row))" :aria-label="en ? 'Select record' : '选择记录'" @update:checked="selectRow(row, $event)" />
...
<style scoped>
.record-card { border: 1px solid var(--n-border-color, #8884); border-radius: 10px; padding: 12px; }
.record-field { display: flex; flex-wrap: wrap; align-items: center; gap: 4px 12px; }
.record-field dt { opacity: 0.65; font-size: 12px; }
.record-field dd { min-width: 0; overflow-wrap: anywhere; }
summary { min-height: 44px; line-height: 44px; cursor: pointer; }
.record-actions :deep(.flex-center) { flex-wrap: wrap; justify-content: flex-start; }
.record-pagination { flex-wrap: wrap; justify-content: center; gap: 8px; }
</style>
