...
watch(visible, async show => {
  ...
  try { info.value = await getAppInfo(); }
...
async function download() {
  ...
  opening.value = true;
  try { await openUrl(release.value.releaseUrl); }
  catch (e) { error.value = String(e); }
...
<template>
  <NModal v-model:show="visible" preset="card" class="app-update-dialog" :title="en ? 'About & updates' : '关于与更新'">
    <div class="flex flex-col gap-16px" aria-live="polite">
      ...
      <NAlert v-if="error" type="error" :title="en ? 'Request failed. Please retry.' : '操作失败，请重试'">{{ error }}</NAlert>
      <template v-if="release">
        <NAlert :type="release.available ? 'info' : 'success'">
          {{ release.available ? (en ? 'New version available' : '发现新版本') : (en ? 'No newer version available' : '当前已无更新版本') }}
      ...
      <NButton :loading="loading" :disabled="loading" @click="check">{{ en ? 'Check for updates' : '检查更新' }}</NButton>
    </div>
  </NModal>
</template>

<style scoped>
.app-update-dialog { width: min(480px, calc(100vw - 24px)); }
</style>
